...
if(isset($_GET['id'])){
    ...
    if (isset($_POST['btn-add'])) {
        if ($team->updateTeam($id, $_POST, $_FILES) > 0) {
            echo "<script>
    ...
    <form action="updateTeam.php?id='.$id.'" method="post" role="form" id="form-add" enctype="multipart/form-data">
        <input type="hidden" name="id" id="id" value="'.$team['team_id'].'">
        <div class="mb-3">
            <label for="team_code" class="form-label">Kode Team</label>
            <input type="text" class="form-control" id="team_code" name="team_code" value="'.$team['team_code'].'" required>
        </div>
        <div class="mb-3">
